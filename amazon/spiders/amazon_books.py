...
class AmazonBooksSpider(Spider):
    ...
    def parse(self, response):
        categories = response.xpath("//span[@class='a-button a-button-span12 bxc-button  bxc-grid__button--light']//a/@href")
    ...
    def parse_categories(self, response):
        ...
        books = response.xpath("//a[@class='a-link-normal s-access-detail-page  s-color-twister-title-link a-text-normal']/@href")
    ...
    def parse_book(self, response):
        ...
        book.add_xpath('year_pub', "//div[@class='content']//li[1]/text()", re=r"\d+")
        book.add_xpath('pages', "//div[@class='content']//text()", re=r"(\d+) páginas")
        ...
        #book.add_xpath('status')

        book.add_value('andress_url', response.url)
        book.add_value('data_scraping', datetime.now())
